...
int intAfterCheck(char *msg)
{
	...
	arith = atoi(temp);//Μόλις βγει από τη while σίγουρα θα έχει πάρει αριθμούς σε όλο το string, 
...
float floatAfterCheck(char *msg)
{
	...
	arith = atof(temp);//Μόλις βγει από τη while σίγουρα θα έχει πάρει αριθμούς σε όλο το string, 
					   //οπότε μετατρέπουμε το string σε αριθμό
	return arith;
}
